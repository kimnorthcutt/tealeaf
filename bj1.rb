...
begin
  ...
  begin
    puts "Would you like another card? Enter y for yes or n for no?"
    response = gets.chomp.downcase
    if response == "y"
      extra_card = deck.pop
      player_bank << extra_card
      player_total += check_card(extra_card)
      puts "Here is you hand #{player_bank}.You got #{extra_card.join(" of ")} and your new total is #{player_total}"
    elsif response == "n"
  ...
  if player_total > 21
    ...
    begin
      ...
      puts "Computer's hand is #{computer_bank}. Computer has #{computer_extra_card.join(" of ")} and its total is #{computer_total}"
  ...
  puts "Would you like to play again? Please enter y for yes and n for no."
